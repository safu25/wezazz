...
<!--        <meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">-->
        ...
        <link href="{{ url('public/img', $settings->favicon) }}" rel="icon">
         ...
         <link href="https://fonts.googleapis.com/css2?family=Mulish:wght@700&family=Nunito:wght@200;300;400;600;700;800&family=Poppins:wght@200;300;400;500;600;700;800&display=swap" rel="stylesheet">
    ...
    <body>
        ...
        <!-- Start Modal payPerEventForm -->
        <div class="modal fade" id="payPerEventForm" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true">
            <div class="modal-dialog modal- modal-dialog-centered modal-sm" role="document">
                <div class="modal-content">
                    <div class="modal-body p-0">
                        <div class="card bg-white shadow border-0">
                            ...
                            <div class="card-body px-lg-5 py-lg-5 position-relative">
                                ...
                                <div class="mb-3">
                                    <i class="feather icon-unlock mr-1"></i> {{trans('general.unlock_interest')}}
                                </div>

                                <form method="post" action="{{url('send/ppe')}}" id="formSendPPE">
                                    ...
                                    <input type="hidden" id="cardholder-name-PPE" value="{{ auth()->user()->name }}"  />
                                    <input type="hidden" id="cardholder-email-PPE" value="{{ auth()->user()->email }}"  />
                                    @csrf
                                    ...
                                    @endphp
                                    <div class="custom-control custom-radio mb-3">
                                        <input name="payment_gateway_ppe" value="{{$payment->id}}" id="ppe_radio{{$payment->id}}" @if ($allPayments->count() == 1 && auth()->user()->wallet == 0.00) checked @endif class="custom-control-input" type="radio">
                                        <label class="custom-control-label" for="ppe_radio{{$payment->id}}">
                                            <span><strong>{!!$paymentName!!}</strong></span>
                                        </label>
                                    </div>

                                    @if ($payment->name == 'Stripe')
                                    <div id="stripeContainerPPE" class="@if ($allPayments->count() != 1) display-none @endif">
                                        <div id="card-elementPPE" class="margin-bottom-10">
                                            <!-- A Stripe Element will be inserted here. -->
                                        </div>
                                        <!-- Used to display form errors. -->
                                        <div id="card-errorsPPE" class="alert alert-danger display-none" role="alert"></div>
                                    </div>
                                    @endif
                                    ...
                                    @if ($settings->disable_wallet == 'on' && auth()->user()->wallet != 0.00 || $settings->disable_wallet == 'off')
                                    <div class="custom-control custom-radio mb-3">
                                        <input name="payment_gateway_ppe" @if (Auth::user()->wallet == 0) disabled @endif value="wallet" id="ppe_radio0" class="custom-control-input" type="radio">
                                        <label class="custom-control-label" for="ppe_radio0">
                                            <span>
                                                <strong>
                                                    <i class="fas fa-wallet mr-1"></i> {{ __('general.wallet') }}
                                                    <span class="w-100 d-block font-weight-light">
                                    ...
                                    <div class="alert alert-danger display-none" id="errorPPE">
                                        <ul class="list-unstyled m-0" id="showErrorsPPE"></ul>
                                    </div>

                                    <div class="text-center">
                                        <button type="submit" id="ppeBtn" class="btn btn-primary mt-4 ppeBtn"><i></i> {{trans('general.pay')}} <span class="pricePPE"></span> <small>{{$settings->currency_code}}</small></button>
...
<!-- model for event create -->
        <div class="modal fade" tabindex="-1" role="dialog" id="eventForm" aria-labelledby="mySmallModalLabel" aria-hidden="true" style="z-index: 1049;">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content wave_content">
                    <div class="modal-header wave_head" style="margin: 0; padding: 0;">
                        
                        <a href="javascript:;" class="event_img position-absolute button-event-upload image-light" onclick="$('#uploadEventImg').trigger('click');" id="event_file" style="z-index: 999;"><!-- margin-left: 30px; {{ auth()->check() && auth()->user()->dark_mode == 'on' ? 'image-dark' : 'image-light' }} -->
                            <i class="fa fa-camera fa-2x" style="font-size: 23px;"></i>
                        </a>
                    ...
                    <div class="modal-body p-0 wave_body">
                        <div class="card border-0">
                            ...
                            <div class="card-body px-lg-5 py-lg-5 position-relative">
                                ...
                                <form method="post" action="{{route('uploadEvent')}}" id="formSendEvent" enctype="multipart/form-data">
...
<!--                                    <input type="file" name="event_image" id="uploadEventImg" accept="image/*" class="visibility-hidden" onchange="return ValidateFileUpload()">-->
                                    ...
                                    <input type="hidden" name="event_images" class="event_images">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-8 col-sm-8">   
                                            <select id="event-cost" name="event_cost" class="form-control browser-default mb-3" required>
                                                <option value="">{{trans('general.event_cost')}}</option>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 col-sm-12">
                                            <div class="form-group" id="event-price" style="display: none;">
                                                <div class="input-group mb-2">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text">$</span>
                                                    </div>
                                                    <input class="form-control isNumber" autocomplete="off" name="event_price" value="event_price" placeholder="Price" type="text" >
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-12 col-sm-6">
                                            <label class="mb-3" for="event_place" style=" padding: .625rem .75rem;">
                                                {{trans('general.event_detail')}} (Optional) : 
                                            </label>
                                        </div> 
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">  
                                            <textarea name="event_detail" id="event_detail" class="form-control" cols="10" rows="5" > </textarea>
                                        </div>
                                    </div>
                                        
                                    <div class="text-center">
                                        <button type="button" class="btn e-none mt-4" data-dismiss="modal">{{trans('admin.cancel')}}</button>
                                        <button type="submit" class="btn btn-primary mt-4" id="submitEventFrm"><i></i> {{trans('general.submit')}}</button>
                                    </div>
                                </form>

                                <div class="alert alert-danger mt-3" id="errorEvent" style="display:none"></div>
