      ...
      <div class="col-lg-12">
        <div class="w-100 text-center py-4 img-profile-user">
          ...
          @endif
            ...
            @if ($user->verified_id == 'yes')
              <button class="btn btn-profile btn-google mr-1" title="{{trans('general.share')}}" id="dropdownUserShare" role="button" data-toggle="modal" data-target=".share-modal">
...
<div class="d-flex-user justify-content-center mb-2" id="showstream">
     
            @foreach ($subscribers as $subscriber)
            
            
          
            @if(auth()->check() && $subscriber->user_id == auth()->user()->id && $user->streaming_status == '1')
        

            
                <a href="{{url('/showLiveStreaming',$user->username)}}" title="{{trans('general.join_streaming')}}" class="btn btn-primary btn-profile">
                       {{trans('general.join_streaming')}}
                </a> <br>
    ...
    @if (session('success_verify'))
      ...
      $(document).ready(function(){
        setInterval(function(){
              $("#showstream").load(window.location.href + " #showstream" );
          //    console.log('check');
        }, 4000);
      });
