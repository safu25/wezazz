...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/cropperjs/1.5.6/cropper.css" integrity="sha256-jKV9n9bkk/CTP8zbtEtnKaKf+ehRovOYeKoyfthwbC8=" crossorigin="anonymous" />
  ...
  <!-- build:css mfb.css -->
    <link href="{{ asset('public/dist/mfb.css') }}" rel="stylesheet">
    <!-- endbuild -->
...
@auth
 ...
 //   var stripeKey = "{{ PaymentGateways::where('id', 2)->where('enabled', '1')->first() ? env('STRIPE_KEY') : false }}";
  var stripeKey = "{{ ($sripe = PaymentGateways::where('id', 2)->where('enabled', '1')->first()) ? $sripe->key : false }}";
...
.icon-navbar { font-size: 23px; vertical-align: bottom; @if (auth()->check() && auth()->user()->dark_mode == 'on') color: #c56cf0 !important; @endif }

{{ $settings->button_style == 'rounded' ? '.btn {border-radius: 50rem!important;}' : null }}

@if (auth()->check() && auth()->user()->dark_mode == 'off' || auth()->guest())
body { background: rgb(2,0,36); background: linear-gradient(180deg, rgba(2,0,36,1) 0%, rgba(246,239,250,1) 0%, rgba(255,255,255,1) 100%) !important; }
