...
//<--- END METHOD
     ...
     * 
     */
    public function users_events(Request $request)
    {
        $query = $request->input('search');

        if($query != '' && strlen($query) > 2)
        {
            $data = Events::where('event_name', 'LIKE', '%' .$query . '%')->orderBy('id','desc')->paginate(20);
        }
        else
        {
            $data =Events::orderBy('id','desc')->paginate(20);
        }
        return view('admin.all_users_events',['data'=>$data, 'query' => $query]);
    }
...
//---> END METHOD
    ...
    public function edit_user_Event($id)
    {
        ...
        return view('admin.edit_user_events')->withData($data);
        
    }

// ---> END METHOD

    public function Update_user_event(Request $request)
    {
        ...
        {
            $path = (public_path('uploads/event/').$events->event_img);
            // $path = config('path.event').$events->event_img;
            if(File::exists($path))
            {
                File::delete($path);
            }
        ...
        return redirect('panel/admin/users_events');
    }

//END METHOD

    public function delete_user_Event(Request $req)
    {
            //return "hey";
        ...
        \Session::flash('success_message', trans('admin.delete_event_msg'));
    ...
    public function banner_images()
    {
       ...
       return view('admin.banner_images',compact('image'));
    }
    //end method

    public function home_ads_Image(Request $request)
    {
        $img = advertisement::find($request->id);
        return view('admin.home_ad',compact('img'));
           
    }

    public function edit_image_banner($id)
    {
      
        $events =advertisement::find($id);
    ...
    public function banner_crop_edit(Request $req)
    {
       // return 'hey';
        $events = advertisement::find($req->event_id);

        //return $events ;
		
		
        $folderPath = config('path.banner');
            $file = "";
            ...
            $success_message = trans('admin.update_msg');
    ...
    public function ad_status_banner($id)
    {
       ...
       $data = DB::table('advertisements')
        ...
        DB::table('advertisements')->where('id',$id)->update($datas);

        $success_message = trans('admin.banner_update');
        return redirect()->back()->with(['success_message' => $success_message]);
    }
    // end method 
...
//<--- End method
    ...
    public function themeStore(Request $request) {
        ...
        if ($request->hasFile('index_image_top')) {
         ...
         if ($request->hasFile('index_image_top_s')) {

            $extension = $request->file('index_image_top_s')->getClientOriginalExtension();
            $file = 'home_index-s-' . time() . '.' . $extension;

            if ($request->file('index_image_top_s')->move($temp, $file)) {
                \File::copy($temp . $file, $path . $file);
                \File::delete($temp . $file);
                // Delete old
                \File::delete($path . $this->settings->home_index_s);
            }// End File

            $this->settings->home_index_s = $file;
            $this->settings->save();
        }// HasFile

         if ($request->hasFile('index_image_top_t')) {

            $extension = $request->file('index_image_top_t')->getClientOriginalExtension();
            $file = 'home_index-t-' . time() . '.' . $extension;

            if ($request->file('index_image_top_t')->move($temp, $file)) {
                \File::copy($temp . $file, $path . $file);
                \File::delete($temp . $file);
                // Delete old
                \File::delete($path . $this->settings->home_index_t);
            }// End File

            $this->settings->home_index_t = $file;
            $this->settings->save();
        }// HasFile
        ...
        $pathevent = 'uploads/default-event-image/';
        $imgPath = 'public/uploads/default-event-image/';

//        set_time_limit(0);
//        ini_set('memory_limit', '512M');

        if ($request->event_default_crop_image != '') {

            $image_parts = explode(";base64,", $request->event_default_crop_image);
            $image_type_aux = explode("image/", $image_parts[0]);
            $image_type = $image_type_aux[1];
            $image_base64 = base64_decode($image_parts[1]);
            ...
            $file = 'event-default-img-' . auth()->user()->id . time() . '.' . $extension;
            ...
            Storage::put($pathevent . $file, $imgCover, 'public');
            \File::delete($imgPath . $this->settings->event_default_image);

            //  file_put_contents($file, $image_base64);

            $this->settings->event_default_image = $file;
            $this->settings->save();
        }
        ...
        $this->settings->whereId(1)
                ->update([
                    ...
                    'home_image_box' => $request->get('home_image_box'),
                    'home_image_desc_box' => $request->get('home_image_desc_box'),
                    'home_image_box_2' => $request->get('home_image_box_2'),
                    'home_image_desc_box_2' => $request->get('home_image_desc_box_2'),
                    'index_1_card_1' => $request->get('index_1_card_1'),
                    'index_1_desc_card_1' => $request->get('index_1_desc_card_1'),
                    'index_2_card_2' => $request->get('index_2_card_2'),
                    'index_2_desc_card_2' => $request->get('index_2_desc_card_2'),
                    'index_3_card_3' => $request->get('index_3_card_3'),
                    'index_3_desc_card_3' => $request->get('index_3_desc_card_3'),
                    'index_4_card_4' => $request->get('index_4_card_4'),
                    'index_4_desc_card_4' => $request->get('index_4_desc_card_4'),
                    'earnings_simulator_title' => $request->get('earnings_simulator_title'),
                    'earnings_simulator_desc' => $request->get('earnings_simulator_desc'),
                    'home_box_title_last' => $request->get('home_box_title_last'),
                    'home_desc_box_last' => $request->get('home_desc_box_last'),
                    'home_image_box_spanish' => $request->get('home_image_box_spanish'),
                    'home_image_desc_box_spanish' => $request->get('home_image_desc_box_spanish'),
                    'home_image_box_2_spanish' => $request->get('home_image_box_2_spanish'),
                    'home_image_desc_box_2_spanish' => $request->get('home_image_desc_box_2_spanish'),
                    'index_1_card_1_spanish' => $request->get('index_1_card_1_spanish'),
                    'index_1_desc_card_1_spanish' => $request->get('index_1_desc_card_1_spanish'),
                    'index_2_card_2_spanish' => $request->get('index_2_card_2_spanish'),
                    'index_2_desc_card_2_spanish' => $request->get('index_2_desc_card_2_spanish'),
                    'index_3_card_3_spanish' => $request->get('index_3_card_3_spanish'),
                    'index_3_desc_card_3_spanish' => $request->get('index_3_desc_card_3_spanish'),
                    'index_4_card_4_spanish' => $request->get('index_4_card_4_spanish'),
                    'index_4_desc_card_4_spanish' => $request->get('index_4_desc_card_4_spanish'),
                    'earnings_simulator_title_spanish' => $request->get('earnings_simulator_title_spanish'),
                    'earnings_simulator_desc_spanish' => $request->get('earnings_simulator_desc_spanish'),
                    'home_box_title_last_spanish' => $request->get('home_box_title_last_spanish'),
                    'home_desc_box_last_spanish' => $request->get('home_desc_box_last_spanish'),
        ]);
...
// End Method
    ...
    //<<--- DATABASE CONTROLLER FOR ENV UPDATE CONNECTIONS---->>>

    public function databasesettings(Request $request){
        ...
        $sql = AdminSettings::first();
        $sql->database_url  = $request->APP_URL;
        $sql->title = $request->app_name;
        $sql->app_env = $request->APP_ENV;
      
        // $sql->app_debug  = $request->APP_DEGUB;
        $sql->save();
...
//		}
    ...
    public function defaultEventImgUpload(Request $request) {

        $path = 'uploads/default-event-image/';
        $imgPath = 'public/uploads/default-event-image/';
...
//        ini_set('memory_limit', '512M');
        ...
        if ($request->event_default_crop_image != '') {
            ...
            Storage::put($path . $file, $imgCover, 'public');
            \File::delete($imgPath . $this->settings->event_default_image);
        ...
        return redirect('panel/admin/eventImgSetting')
                        ->with('success_message', trans('admin.success_update'));
    }
