...
class PayPerEventController extends Controller {
    ...
    public function send() {
        ...
        if ($this->request->isEvent) {
            // Find Message
            $media = Events::whereId($this->request->id)->whereEventPrice($this->request->amount)->where('user_id', '<>', auth()->user()->id)->firstOrFail();

            // Verify that the user has not purchased the content
            if (PayPerViews::whereUserId(auth()->user()->id)->whereEventsId($this->request->id)->first()) {
        ...
        $messages = [
            'payment_gateway_ppe.required' => trans('general.choose_payment_gateway')
        ];

        //<---- Validation
        $validator = Validator::make($this->request->all(), [
                    'payment_gateway_ppe' => 'required'
                        ], $messages);
        ...
        switch ($this->request->payment_gateway_ppe) {
            case 'wallet':
...
// End method Send
    ...
    protected function sendWallet() {
        ...
        $media = Events::find($this->request->id);
        ...
        $this->transaction('ppe_' . str_random(25), auth()->user()->id, 0, $media->user()->id, $amount, $earnings['user'], $earnings['admin'], 'Wallet', 'ppe', $earnings['percentageApplied']);
        ...
        if ($this->request->isEvent) {
            ...
            $this->payPerEvents(auth()->user()->id, 0, 0, $this->request->id);
            $url = url('events');

            // Send Notification - destination, author, type, target
            Notifications::send($media->user()->id, auth()->user()->id, '9', $this->request->id);
        }
...
// End sendWallet
    ...
    protected function sendPayPal() {
        ...
        if ($this->request->isEvent) {
            $urlSuccess = url('events');
            $urlCancel = url('events');
            $isMessage = true;
        }

        $urlPaypalIPN = url('paypal/ppe/ipn');
        ...
        return response()->json([
                  ...
                  <input type="hidden" name="item_name" value="' . __('general.unlock_interest') . ' @' . $media->user()->username . '">
        ...
        if ($verified) {
            if ($payment_status == 'Completed') {
                ...
                if (!isset($verifiedTxnId)) {
                    ...
                    // Insert Transaction
                    // $txnId, $userId, $subscriptionsId, $subscribed, $amount, $userEarning, $adminEarning, $paymentGateway, $type, $percentageApplied
                    $this->transaction($txn_id, $data['sender'], 0, $media->user()->id, $data['amount'], $earnings['user'], $earnings['admin'], 'PayPal', 'ppe', $earnings['percentageApplied']);
                    ...
                    if ($data['m'] != '') {
                        // $user_id, $updates_id, $messages_id
                        $this->payPerEvents($data['sender'], 0, 0, $data['id']);

                        // Send Notification - destination, author, type, target
                        Notifications::send($media->user()->id, $data['sender'], '9', $data['id']);
                    }
                }// <--- Verified Txn ID
            } // <-- Payment status
        } else {
...
//<----- End Method paypalIpn()
    ...
    protected function sendStripe() {
        ...
        $description = __('general.unlock_interest') . ' @' . $media->user()->username;
...
// End Method sendStripe
    ...
    protected function generatePaymentResponse($intent) {
        ...
        if (isset($intent->status) && $intent->status == 'requires_action' &&
            ...
            // Insert Transaction
            // $txnId, $userId, $subscriptionsId, $subscribed, $amount, $userEarning, $adminEarning, $paymentGateway, $type, $percentageApplied
            $this->transaction('ppe_' . str_random(25), auth()->user()->id, 0, $media->user()->id, $amount, $earnings['user'], $earnings['admin'], 'Stripe', 'ppe', $earnings['percentageApplied']);
        ...
        if (isset($this->request->trxref)) {
            ...
            if ('success' === $tranx->data->status) {
                ...
                if (!isset($verifyTxnId)) {
                    ...
                    // Insert Transaction
                    // $txnId, $userId, $subscriptionsId, $subscribed, $amount, $userEarning, $adminEarning, $paymentGateway, $type, $percentageApplied
                    $this->transaction('ppe_' . str_random(25), auth()->user()->id, 0, $media->user()->id, $amount, $earnings['user'], $earnings['admin'], 'Paystack', 'ppe', $earnings['percentageApplied']);
...
// end method
    ...
    public function sendCCbill() {
        ...
        $input['type'] = 'ppe';
        $input['isMessage'] = $this->request->isEvent ?? null;
